...
interface CarInteractionBounds {
    ...
    distanceSqToCenter: number;
    pointerInsideBody: boolean;
}
const COLORS = {
    bg: 0xf3f8fb,
    grid: 0xd9e6ef,
...
export class ParkingJamGameScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.resetTelemetry();
        this.effectiveLimitMs = Math.min(
            PARKING_JAM_SESSION_MS,
            this.level.timeLimitMs ?? PARKING_JAM_SESSION_MS,
        );
    ...
    private createControls() {
        this.controls.undo = this.createControlButton("ย้อนกลับ", () =>
            this.handleUndo(),
        );
        this.controls.reset = this.createControlButton("รีเซ็ต", () =>
            this.handleReset(),
        );
    }
    ...
    private createControlButton(label: string, onClick: () => void) {
        ...
        const bg = this.add
            .rectangle(0, 0, 120, 56, COLORS.buttonBg, 1)
    ...
    private getPointerWorldPoint(pointer: Phaser.Input.Pointer) {
        const world = pointer.positionToCamera(this.cameras.main) as {
            x: number;
            y: number;
        } | null;
        if (world) {
    ...
    private getCarAtPointer(point: Phaser.Math.Vector2): CarVisual | null {
        ...
        this.cars.forEach((car) => {
            ...
            const {
                ...
                distanceSqToCenter,
                pointerInsideBody,
            } = this.getCarInteractionBounds(car, point);
        ...
        candidates.sort((a, b) => {
            if (a.pointerInsideBody !== b.pointerInsideBody) {
                return a.pointerInsideBody ? -1 : 1;
            }

            if (a.distanceSqToCenter !== b.distanceSqToCenter) {
                return a.distanceSqToCenter - b.distanceSqToCenter;
            }
    ...
    private getBoardDensityMode(): BoardDensityMode {
        const denseByGrid =
            this.level.gridSize >= 6 && this.level.cars.length >= 8;
    ...
    private getCarVisualDensity(): CarVisualDensity {
        ...
        if (this.cellSize <= 66)
            return mode === "compact" ? "compact" : "medium";
    ...
    private getCarPixelBounds(config: ParkingJamCarConfig): CarPixelBounds {
        ...
        return {
            ...
            heightPx,
            cornerRadius:
                density === "compact" ? 12 : density === "medium" ? 13 : 14,
            labelInsetX:
                density === "compact" ? 8 : density === "medium" ? 10 : 12,
            labelInsetY:
                density === "compact" ? 7 : density === "medium" ? 9 : 10,
        };
    }
    ...
    private getCarInteractionBounds(
        car: CarVisual,
        point?: Phaser.Math.Vector2,
    ): CarInteractionBounds {
        const bodyRect = this.getCarBodyRect(car);
        const occupiedRect = this.getCarOccupiedRect(car);

        const minTouchTarget = 44;
        const extraWidth = Math.max(0, minTouchTarget - bodyRect.width);
        const extraHeight = Math.max(0, minTouchTarget - bodyRect.height);

        const touchPaddingX = Math.max(10, Math.ceil(extraWidth / 2));
        const touchPaddingY = Math.max(14, Math.ceil(extraHeight / 2) + 4);

        const boardLeft = this.gridOriginX;
        const boardTop = this.gridOriginY;
        const boardRight =
            this.gridOriginX + this.level.gridSize * this.cellSize;
        const boardBottom =
            this.gridOriginY + this.level.gridSize * this.cellSize;

        const touchLeft = Math.max(
            boardLeft,
            Math.min(bodyRect.x, occupiedRect.x) - touchPaddingX,
        );
        const touchTop = Math.max(
            boardTop,
            Math.min(bodyRect.y, occupiedRect.y) - touchPaddingY,
        );
        const touchRight = Math.min(
            boardRight,
            Math.max(
                bodyRect.x + bodyRect.width,
                occupiedRect.x + occupiedRect.width,
            ) + touchPaddingX,
        );
        const touchBottom = Math.min(
            boardBottom,
            Math.max(
                bodyRect.y + bodyRect.height,
                occupiedRect.y + occupiedRect.height,
            ) + touchPaddingY,
        );

        const centerX = bodyRect.centerX;
        const centerY = bodyRect.centerY;
        const dx = (point?.x ?? centerX) - centerX;
        const dy = (point?.y ?? centerY) - centerY;
        ...
        return {
            ...
            distanceSqToCenter: dx * dx + dy * dy,
            pointerInsideBody: point
                ? bodyRect.contains(point.x, point.y)
                : false,
        };
    }
    ...
    private layoutScene() {
        ...
        this.cellSize = Math.floor(
            Math.min(
                ...
                Math.max(
                    40,
                    (boardBottomLimit - boardTop) / this.level.gridSize,
                ),
            ),
        );
        ...
        this.gridOriginY = Math.floor(
            (boardTop + boardBottomLimit - boardSize) / 2,
        );

        this.objectiveText
            .setPosition(width / 2, metrics.objectiveY)
            .setFontSize(metrics.objectiveFontSize);
        this.progressText
            .setPosition(width / 2, metrics.progressY)
            .setFontSize(metrics.progressFontSize);
        ...
        const totalWidth =
            buttons.length * buttonWidth + (buttons.length - 1) * gap;
        const startX = (width - totalWidth) / 2 + buttonWidth / 2;
        const y = metrics.buttonY;
        buttons.forEach((button, index) => {
            this.applyControlButtonLayout(
                button,
                metrics.buttonWidth,
    ...
    private drawBoard() {
        ...
        const blockedCells = new Set(
            this.level.blockedCells.map((cell) => `${cell.row},${cell.col}`),
        );

        this.levelGraphics.fillStyle(0xffffff, 1);
        this.levelGraphics.fillRoundedRect(
            this.gridOriginX - 12,
            this.gridOriginY - 12,
            boardSize + 24,
            boardSize + 24,
        ...
        this.levelGraphics.lineStyle(
            metrics.boardBorderWidth,
            COLORS.gridBorder,
            1,
        );
        this.levelGraphics.strokeRoundedRect(
            this.gridOriginX - 12,
            this.gridOriginY - 12,
            boardSize + 24,
            boardSize + 24,
    ...
    private drawGate(edge: "top" | "right" | "bottom" | "left", index: number) {
        ...
        const boardSize = this.cellSize * this.level.gridSize;
        const gateThickness = Math.max(
            metrics.gateMinThickness,
            Math.round(this.cellSize * 0.14),
        );
        const gateInset = Math.max(
            metrics.gateMinInset,
            Math.round(this.cellSize * 0.13),
        );
        const gateLength = this.cellSize - gateInset * 2;
        const gateRadius = Math.max(4, Math.round(gateThickness * 0.55));

        this.levelGraphics.fillStyle(
            color,
            blocked ? metrics.blockedGateAlpha : metrics.openGateAlpha,
        );
        ...
        if (edge === "left") {
            ...
            this.levelGraphics.fillRoundedRect(
                x + Math.max(1, Math.floor(gateThickness * 0.28)),
                y + 3,
                Math.max(2, Math.floor(gateThickness * 0.32)),
                Math.max(6, gateLength - 6),
                2,
            );
        } else if (edge === "right") {
            ...
            this.levelGraphics.fillRoundedRect(
                x + 3,
                y + Math.max(1, Math.floor(gateThickness * 0.28)),
                Math.max(6, gateLength - 6),
                Math.max(2, Math.floor(gateThickness * 0.32)),
                2,
            );
        ...
        if (edge === "left" || edge === "right") {
            ...
            const centerY =
                this.gridOriginY + index * this.cellSize + this.cellSize * 0.5;
        ...
        const centerX =
            this.gridOriginX + index * this.cellSize + this.cellSize * 0.5;
        const centerY =
            edge === "top"
    ...
    private drawConeInCell(row: number, col: number) {
        ...
        this.levelGraphics.fillStyle(0xf97316, 0.98);
        this.levelGraphics.beginPath();
        this.levelGraphics.moveTo(
            centerX - coneBaseW * 0.5,
            centerY + coneH * 0.5,
        );
        this.levelGraphics.lineTo(
            centerX - coneTopW * 0.5,
            centerY - coneH * 0.5,
        );
        this.levelGraphics.lineTo(
            centerX + coneTopW * 0.5,
        ...
        this.levelGraphics.closePath();
        this.levelGraphics.fillPath();

        this.levelGraphics.fillStyle(0xffedd5, 0.96);
        ...
        this.levelGraphics.fillStyle(0x7c2d12, 0.28);
        this.levelGraphics.fillEllipse(
            centerX,
            centerY + coneH * 0.56,
            coneBaseW * 0.9,
            Math.max(4, coneH * 0.16),
        );
    }
    ...
    private renderCar(car: CarVisual) {
        ...
        car.bg.lineStyle(
            density === "compact" ? 1.5 : 2,
            0x0f172a,
            density === "compact" ? 0.22 : 0.28,
        );
        car.bg.strokeRoundedRect(
            -widthPx / 2,
            -heightPx / 2,
            widthPx,
            heightPx,
        ...
        car.label
            .setPosition(
                -widthPx / 2 + labelInsetX,
                -heightPx / 2 + labelInsetY,
            )
            .setOrigin(0, 0)
            .setFontSize(
                density === "compact" ? 13 : density === "medium" ? 14 : 16,
            );
    ...
    private drawVehicleDetails(
        ...
        const density = this.getCarVisualDensity();
        g.clear();
        const defaultForward: ParkingJamDirection =
            axis === "h" ? "right" : "down";
        let forwardDirection = (car.config.allowedExitDirections[0] ??
            defaultForward) as ParkingJamDirection;
        if (
            axis === "h" &&
            (forwardDirection === "up" || forwardDirection === "down")
        ) {
        ...
        if (!isTwoWay) {
            if (axis === "h") {
                roofX +=
                    forwardDirection === "right" ? -roofShiftPx : roofShiftPx;
                roofX = Phaser.Math.Clamp(
                    roofX,
                    -widthPx / 2 + 2,
                    widthPx / 2 - roofWidth - 2,
                );
            } else {
                roofY +=
                    forwardDirection === "down" ? -roofShiftPx : roofShiftPx;
                roofY = Phaser.Math.Clamp(
                    roofY,
                    -heightPx / 2 + 2,
                    heightPx / 2 - roofHeight - 2,
                );
            }
        }
        ...
        const lightThickness = Math.max(
            6,
            Math.floor(Math.min(widthPx, heightPx) * 0.18),
        );
        ...
        if (isTwoWay) {
            ...
            if (axis === "h") {
                ...
                g.fillRoundedRect(
                    ...
                    horizontalLightY,
                    lightThickness,
                    horizontalLightHeight,
                    4,
                );
                g.fillRoundedRect(
                    leftX,
                    -horizontalLightY - horizontalLightHeight,
                    lightThickness,
                    horizontalLightHeight,
                    4,
                );
                ...
                const leftCoreX =
                    leftX +
                    Math.max(0, Math.floor((lightThickness - coreWidth) * 0.5));
                const rightCoreX =
                    rightX +
                    Math.max(0, Math.floor((lightThickness - coreWidth) * 0.5));
                ...
                g.fillRoundedRect(
                    -verticalLightX - verticalLightWidth,
                    topY,
                    verticalLightWidth,
                    lightThickness,
                    4,
                ...
                const leftCoreX =
                    ...
                    Math.max(
                        0,
                        Math.floor((verticalLightWidth - coreWidth) * 0.5),
                    );
                const rightCoreX =
                    -verticalLightX -
                    verticalLightWidth +
                ...
                const topCoreY =
                    ...
                    Math.max(
                        0,
                        Math.floor((lightThickness - coreHeight) * 0.5),
                    );
                const bottomCoreY =
                    bottomY +
                    Math.max(
            ...
            const frontX = frontOnRight
                ? widthPx / 2 - lightThickness - 1
                : -widthPx / 2 + 1;
            const rearX = frontOnRight
                ? -widthPx / 2 + 1
                : widthPx / 2 - lightThickness - 1;
            ...
            const whiteCoreX =
                frontX +
                Math.max(0, Math.floor((lightThickness - whiteCoreW) * 0.5));
            ...
            const frontY = frontOnBottom
                ? heightPx / 2 - lightThickness - 1
                : -heightPx / 2 + 1;
            const rearY = frontOnBottom
                ? -heightPx / 2 + 1
                : heightPx / 2 - lightThickness - 1;
            g.fillStyle(brightHeadlight, 0.98);
            g.fillRoundedRect(
            ...
            const leftCoreX =
                ...
                Math.max(
                    0,
                    Math.floor((verticalLightWidth - whiteCoreW) * 0.5),
                );
            const rightCoreX =
                -verticalLightX -
                verticalLightWidth +
            ...
            const coreY =
                frontY +
                Math.max(0, Math.floor((lightThickness - whiteCoreH) * 0.5));
            g.fillStyle(headlightCore, 0.96);
        ...
        g.lineStyle(
            density === "compact" ? 0.8 : density === "medium" ? 1 : 1.5,
            0xffffff,
            density === "compact" ? 0.42 : density === "medium" ? 0.5 : 0.6,
        );
        if (axis === "h") {
            const windshieldWidth =
                density === "compact"
                    ? Math.max(8, Math.round(widthPx * 0.16))
                    : Math.max(8, Math.round(roofWidth * 0.2));
            const windshieldHeight =
                density === "compact"
                    ? Math.max(10, Math.round(heightPx * 0.5))
                    : Math.max(8, roofHeight - 6);
            const frontInset =
                density === "compact"
                    ? Math.max(5, Math.round(widthPx * 0.05))
                    : 4;
            const windshieldY =
                density === "compact" ? -windshieldHeight / 2 : roofY + 3;

            if (isTwoWay) {
                const leftWindshieldX =
                    density === "compact"
                        ? -widthPx / 2 + frontInset
                        : roofX + frontInset;
                const rightWindshieldX =
                    density === "compact"
                ...
                g.fillRoundedRect(
                    ...
                    windshieldWidth,
                    windshieldHeight,
                    density === "compact" ? 2 : density === "medium" ? 2 : 3,
                );
                g.beginPath();
                g.moveTo(
                    leftWindshieldX + windshieldWidth - 1,
                    windshieldY + 1,
                );
                ...
                g.lineTo(
                    rightWindshieldX + windshieldWidth - 1,
                    windshieldY + 4,
                );
                g.strokePath();
            } else {
            ...
            const frontInset =
                density === "compact"
                    ? Math.max(5, Math.round(heightPx * 0.05))
                    : 4;
            const windshieldX =
                density === "compact" ? -windshieldWidth / 2 : roofX + 3;

            if (isTwoWay) {
                const topWindshieldY =
                    density === "compact"
                        ? -heightPx / 2 + frontInset
                        : roofY + frontInset;
                const bottomWindshieldY =
                    density === "compact"
                ...
                g.moveTo(
                    windshieldX + 1,
                    topWindshieldY + windshieldHeight - 1,
                );
                ...
                g.lineTo(
                    windshieldX + 4,
                    bottomWindshieldY + windshieldHeight - 1,
                );
                g.strokePath();
            } else {
                const windshieldY =
                ...
                if (forwardDirection === "down") {
                    ...
                    g.lineTo(
                        windshieldX + 4,
                        windshieldY + windshieldHeight - 1,
                    );
                } else {
                    g.moveTo(
                        windshieldX + 1,
        ...
        const tireSize = Math.max(
            density === "medium" ? 4 : 5,
            Math.floor(
                Math.min(widthPx, heightPx) *
                    (density === "medium" ? 0.12 : 0.15),
            ),
        );
        if (axis === "h") {
            g.fillRoundedRect(
    ...
    private drawDirectionHints(
        ...
        directions.forEach((direction) => {
            ...
            const marker = Math.max(
                ...
                Math.floor(
                    Math.min(widthPx, heightPx) *
                        (density === "compact" ? 0.2 : 0.25),
                ),
            );

            g.fillStyle(color, alpha);
            g.lineStyle(
                density === "compact" ? 2.2 : 3,
                0xffffff,
                allowed ? 0.9 : 0.55,
            );

            const inset = density === "compact" ? 5 : 7;
            if (direction === "left") {
                g.beginPath();
            ...
            if (allowed) return;
            g.lineStyle(density === "compact" ? 2.2 : 3, 0x7f1d1d, 0.9);
            if (direction === "left") {
                g.beginPath();
    ...
    private handleCarTap(carId: string, point: Phaser.Math.Vector2) {
        ...
        if (this.getTapZoneForPointer(car, point) === "center") {
            this.toast(
                car.config.axis === "h"
                    ? "แตะซ้ายหรือขวาของรถคันนี้"
                    : "แตะบนหรือล่างของรถคันนี้",
                false,
            );
        }
    }
    ...
    private handleSelectedCarTap(car: CarVisual, point: Phaser.Math.Vector2) {
        ...
        if (zone === "center") {
            this.toast(
                car.config.axis === "h"
                    ? "แตะครึ่งซ้ายหรือขวาเพื่อเลือกทิศ"
                    : "แตะครึ่งบนหรือล่างเพื่อเลือกทิศ",
                true,
            );
            return;
        }
    ...
    private trySelectedDirection(
        ...
        this.showSelectionFeedback(
            car,
            direction,
            this.isDirectionCurrentlyAvailable(car, direction),
        );
        this.clearSelectedCar();
    ...
    private tryStepMove(
        ...
        const distance =
            sign < 0 ? Math.abs(slideRange.minOffset) : slideRange.maxOffset;
        ...
        if (this.canExitByDirection(car, direction)) {
            ...
            this.carMoveHistogram[car.config.id] =
                (this.carMoveHistogram[car.config.id] ?? 0) + 1;
        ...
        this.tweens.add({
            ...
            x:
                this.gridOriginX +
                (car.runtime.col +
                    (car.config.axis === "h" ? car.config.length / 2 : 0.5)) *
                    this.cellSize,
            y:
                this.gridOriginY +
                (car.runtime.row +
                    (car.config.axis === "v" ? car.config.length / 2 : 0.5)) *
                    this.cellSize,
            duration: tweenMs,
            ease: "Sine.out",
            onComplete: () => {
    ...
    private computeSlideRange(car: CarVisual) {
        ...
        if (config.axis === "h") {
            ...
            while (
                runtime.col + config.length - 1 + maxOffset + 1 <
                this.level.gridSize
            ) {
                const checkCol =
                    runtime.col + config.length - 1 + maxOffset + 1;
        ...
        while (
            runtime.row + config.length - 1 + maxOffset + 1 <
            this.level.gridSize
        ) {
        ...
        this.cars.forEach((car) => {
            ...
            for (let r = 0; r < heightCells; r += 1) {
                for (let c = 0; c < widthCells; c += 1) {
                    occupied.add(
                        `${car.runtime.row + r},${car.runtime.col + c}`,
                    );
                }
            }
        });
    ...
    private isBlockedExitAttempt(
        ...
        if (direction === "left" && car.runtime.col === 0)
            return !this.canExitByDirection(car, direction);
        if (
            direction === "right" &&
            car.runtime.col + car.config.length - 1 === this.level.gridSize - 1
        ) {
            return !this.canExitByDirection(car, direction);
        }
        if (direction === "up" && car.runtime.row === 0)
            return !this.canExitByDirection(car, direction);
        if (
            direction === "down" &&
            car.runtime.row + car.config.length - 1 === this.level.gridSize - 1
        ) {
            return !this.canExitByDirection(car, direction);
        }
        return false;
    ...
    private checkObjectiveCompletion() {
        const allRemoved = [...this.cars.values()].every(
            (car) => car.runtime.removed,
        );
        if (allRemoved) {
            this.completeLevel({
                solved: true,
    ...
    private calculateStars(
        ...
        const timeRatio =
            attempt.levelTimeMs / Math.max(1, this.level.parTimeMs);
        const errorPenalty =
        ...
        if (moveRatio <= 1.4 && timeRatio <= 1.35 && errorPenalty <= 1)
            return 3;
    ...
    private updateProgressText() {
        const cleared = [...this.cars.values()].filter(
            (car) => car.runtime.removed,
        ).length;
    ...
    private drawTimerDial() {
        ...
        const pct = Math.max(
            0,
            Math.min(1, this.remainingMs / this.effectiveLimitMs),
        );
    ...
    private renderSelectedOverlay(car: CarVisual) {
        ...
        const density = this.getCarVisualDensity();
        const { bodyRect } = this.getCarInteractionBounds(car, undefined);
        ...
        directions.forEach((direction) => {
            const available = this.isDirectionCurrentlyAvailable(
                car,
                direction,
            );
            g.fillStyle(
                available ? 0x67e8f9 : 0x94a3b8,
                available ? 0.22 : 0.14,
            );
            ...
            const offset = Math.max(
                density === "compact" ? 15 : 19,
                Math.min(widthPx, heightPx) *
                    (density === "compact" ? 0.2 : 0.26),
            );
            const arrowX =
                direction === "left"
                    ? centerX - offset
            ...
            this.drawSelectionArrow(
                ...
                direction,
                Math.max(
                    density === "compact" ? 12 : 15,
                    Math.floor(
                        Math.min(widthPx, heightPx) *
                            (density === "compact" ? 0.21 : 0.25),
                    ),
                ),
            );
        });
    ...
    private getTapZoneForPointer(
        car: CarVisual,
        point: Phaser.Math.Vector2,
    ): TapZone {
        const { touchRect } = this.getCarInteractionBounds(car, point);
    ...
    private isDirectionCurrentlyAvailable(
        car: CarVisual,
        direction: ParkingJamDirection,
    ) {
    ...
    private showSelectionFeedback(
        ...
        const draw = (alpha: number) => {
            ...
            this.selectionFeedbackGraphics.fillStyle(
                success ? 0x22c55e : 0xef4444,
                alpha,
            );
            if (car.config.axis === "h") {
                const zoneLeft = direction === "left" ? left : centerX;
                this.selectionFeedbackGraphics.fillRect(
                    zoneLeft,
                    top,
                    widthPx / 2,
                    heightPx,
    ...
    private setupAudio() {
        try {
            ...
            this.blockedSfx = this.sound.add(SOUND_KEYS.blocked, {
                volume: 0.4,
            });
            this.exitSfx = this.sound.add(SOUND_KEYS.exit, { volume: 0.45 });
            this.levelPassSfx = this.sound.add(SOUND_KEYS.levelPass, {
                volume: 0.55,
            });
            this.levelFailSfx = this.sound.add(SOUND_KEYS.levelFail, {
                volume: 0.55,
            });
            ...
            this.bgMusic = this.sound.add(SOUND_KEYS.bgm, {
                loop: true,
                volume: 0.28,
            });
            this.bgMusic.play();
    ...
    private showDirectionGuidePopup() {
        ...
        const scrim = this.add
            .rectangle(0, 0, width, height, 0x0f172a, 0.52)
            .setOrigin(0)
            .setInteractive();
        ...
        const panelTapZone = this.add
            .zone(width / 2, height / 2, panelWidth, panelHeight)
            .setOrigin(0.5)
            .setInteractive({ useHandCursor: true });
        ...
        const example = this.createDirectionGuideExample(
            width / 2,
            height / 2 - panelHeight * 0.14,
        );
    ...
    private readReduceMotion() {
        try {
            ...
            return (
                window.localStorage.getItem("parking_jam_reduce_motion") === "1"
            );
        } catch {
            return false;
        }
    ...
    private persistReduceMotion() {
        try {
            ...
            window.localStorage.setItem(
                "parking_jam_reduce_motion",
                this.reduceMotion ? "1" : "0",
            );
        } catch {
