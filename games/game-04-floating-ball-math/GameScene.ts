...
export class FloatingBallMathGameScene extends Phaser.Scene {
    ...
    private completedEquationStats: {
        target: number;
    ...
    init(data: { level: number }) {
        ...
        this.rng = createSeededRandom(
            4000 + this.currentLevelConfig.level * 911,
        );
    ...
    resetGameState() {
        ...
        if (this.activeWarningTimerEvent) {
            ...
            this.activeWarningTimerEvent = null;
        }
        this.balls = [];
    ...
    preload() {
        ...
        this.load.image(
            "bomb-ball",
            "/assets/images/floatingBallMath/Bomb.png",
        );
        ...
        this.load.audio(
            "ball-collect",
            "/assets/sounds/floatingball-math/waterspalsh.mp3",
        );
        ...
        this.load.audio(
            "bg-music",
            "/assets/sounds/floatingball-math/bg-music.mp3",
        );
        ...
        this.load.audio(
            "timer-tick",
            "/assets/sounds/floatingball-math/timertick.mp3",
        );
    }
    ...
    create() {
        ...
        if (floatboat) {
            ...
            const boatSprite = floatboat.container.getByName(
                "boat",
        ...
        try {
            this.soundBallCollect = this.sound.add("ball-collect", {
                volume: 0.5,
            });
    ...
    private cleanupAudio() {
        ...
        sounds.forEach((sound) => {
            ...
            try {
                ...
                console.warn("Failed to destroy sound", e);
            }
        });
    }
    /**
    ...
    private handleBoatDragStart(pointer: Phaser.Input.Pointer) {
        if (!this.isDraggingBoat) {
            ...
            console.log(
                `[FloatingBallMathGameScene] Boat drag started at X: ${pointer.x}`,
            );
        }
    }
    ...
    private handleBoatDrag(pointer: Phaser.Input.Pointer) {
        if (this.isDraggingBoat) {
            ...
            if (floatboat) {
                ...
                floatboat.container.x = Phaser.Math.Linear(
                    floatboat.container.x,
                    clampedX,
                    lerpFactor,
                );
    ...
    private handleBoatDragEnd(pointer: Phaser.Input.Pointer) {
        if (this.isDraggingBoat) {
            this.isDraggingBoat = false;
            console.log(
                `[FloatingBallMathGameScene] Boat drag ended at X: ${pointer.x}`,
            );
    ...
    private snapBoatToNearestLane(currentX: number) {
        ...
        const distances = [
            {
                lane: 0,
                center: this.lanes.left,
                distance: Math.abs(currentX - this.lanes.left),
            },
            {
                lane: 1,
                center: this.lanes.center,
                distance: Math.abs(currentX - this.lanes.center),
            },
            {
                lane: 2,
                center: this.lanes.right,
                distance: Math.abs(currentX - this.lanes.right),
            },
        ];
        ...
        const distances = [
            {
                lane: 0,
                center: this.lanes.left,
                distance: Math.abs(x - this.lanes.left),
            },
            {
                lane: 1,
                center: this.lanes.center,
                distance: Math.abs(x - this.lanes.center),
            },
            {
                lane: 2,
                center: this.lanes.right,
                distance: Math.abs(x - this.lanes.right),
            },
        ];
    ...
    createWaterBackground() {
        ...
        this.waterBackground.fillGradientStyle(
            0xe3f2fd,
            0xe3f2fd,
            0xbbdefb,
            0xbbdefb,
    ...
    private showWarningToast(message: string) {
        ...
        this.activeWarningTimerEvent = this.time.delayedCall(
            this.warningDisplayTime,
            () => {
                this.hideWarningToast();
            },
        );
    ...
    private moveBoatToLane(lane: 0 | 1 | 2) {
        ...
        if (now - this.lastLaneMoveTime < this.laneMoveCooldown) {
            console.log(
                `[FloatingBallMathGameScene] Lane move on cooldown (${this.laneMoveCooldown}ms)`,
            );
            return;
        }
    ...
    generateNewEquation() {
        ...
        try {
            ...
            console.log(
                "[FloatingBallMathGameScene] New equation:",
            ...
            console.log(
                "[FloatingBallMathGameScene] Balls will be spawned via generateSolvableBallSet() and replacement spawning",
            );
        } catch (error) {
            console.error(
                "[FloatingBallMathGameScene] ERROR in generateNewEquation:",
                error,
            );
        }
    }
    ...
    private generateSolvableBallSet() {
        console.log(
            "[FloatingBallMathGameScene] Generating solvable ball set for target:",
        ...
        ballTemplates.forEach((ballTemplate, index) => {
            ...
            if (ballTemplate.isBomb) {
                ballObj = this.ballSpawner.createBombBall(
                    ballTemplate.value,
            ...
            // Fade in ball
            const targetBall = ballTemplate.isBomb
                ? this.balls[this.balls.length - 1]
                : ballObj;
            ...
            console.log(
                `[FloatingBallMathGameScene] Spawned ${ballTemplate.isBomb ? "bomb" : ballTemplate.isSolvable ? "solvable" : "distractor"} ball: ${ballTemplate.operator} ${ballTemplate.value} in lane ${lane}`,
            );
        });

        console.log(
            `[FloatingBallMathGameScene] Spawned ${ballTemplates.length} balls total`,
        );
    }
    ...
    private spawnReplacementSolvableBall() {
        console.log(
            "[FloatingBallMathGameScene] Spawning replacement solvable ball",
        );
        ...
        const solvableBalls = ballTemplates.filter(
            (ball) => ball.isSolvable && !ball.isBomb,
        );

        if (solvableBalls.length > 0) {
            // Select 1 random solvable ball from generated set
            const ballTemplate =
                solvableBalls[this.rng.nextIndex(solvableBalls.length)];
        ...
        console.log(
            "[FloatingBallMathGameScene] No solvable balls from generator, creating fallback solution ball",
        );
        ...
        console.log(
            `[FloatingBallMathGameScene] Spawned FALLBACK solvable ball: ${ballObj.operator} ${ballObj.value}`,
        );
    }
    ...
    private spawnBallFromTemplate(ballTemplate: any) {
        ...
        console.log(
            `[FloatingBallMathGameScene] Spawned replacement solvable ball: ${ballObj.operator} ${ballObj.value} in lane ${lane}`,
        );
    }
    ...
    private endGameWithLoss() {
        console.log(
            "[FloatingBallMathGameScene] Game over - score dropped below 0",
        );
    ...
    private cleanupOffScreenBalls() {
        ...
        this.balls = this.balls.filter((ball) => {
            ...
            if (ball && ball.container && ball.y > bottomThreshold) {
                ...
                if (ball.isSolvable) {
                    console.log(
                        "[FloatingBallMathGameScene] Solvable ball went off-screen, spawning replacement",
                    );
    ...
    private findSafeSpawnPosition(lane: 0 | 1 | 2): number {
        ...
        const laneBalls = this.balls.filter(
            (ball) =>
                ball &&
                ball.container &&
                !ball.isCollected &&
    ...
    startThiefSpawning() {
        ...
        console.log(
            `[FloatingBallMathGameScene] Started thief spawning every ${spawnInterval}ms (level ${this.currentLevelConfig.level})`,
        );
    }
    ...
    spawnThiefEvent() {
        ...
        // Pick random ball
        const targetBall =
            availableBalls[this.rng.nextIndex(availableBalls.length)];

        // Select new lane (different from current)
        const possibleLanes = [0, 1, 2].filter((l) => l !== targetBall.lane);
        const newLane = possibleLanes[
            this.rng.nextIndex(possibleLanes.length)
        ] as 0 | 1 | 2;
        ...
        console.log(
            `[FloatingBallMathGameScene] Thief spawned - ball ${targetBall.id}, lane ${targetBall.lane} -> ${newLane}`,
        );
    }
        ...
        this.blockTimerEvent = this.time.addEvent({
            ...
            callback: () => {
                ...
                const remainingMs = Math.max(
                    0,
                    this.activeThiefEvent.decisionWindowMs - elapsed,
                );
                const pct =
                    (remainingMs / this.activeThiefEvent.decisionWindowMs) *
                    100;
    ...
    private drawBlockTimerBar(pct: number) {
        ...
        if (pct > 0) {
            ...
            this.blockTimerBar.fillRoundedRect(
                x,
                y,
                barW * (pct / 100),
                barH,
    ...
    private flashTargetDisplayGreen() {
        console.log(
            "[FloatingBallMathGameScene] Flashing target display green",
        );

        // Get the target background graphics (second element in container)
        const targetBg = this.targetDisplay.getAt(
            1,
    ...
    handleBlockDecision() {
        ...
        console.log(
            "[FloatingBallMathGameScene] Player chose BLOCK - canceling thief",
        );

        const isOnTime =
            Date.now() - this.activeThiefEvent.appearTime <
            this.activeThiefEvent.decisionWindowMs;
    ...
    executeAdapt() {
        ...
        const ball = this.balls.find(
            (b) => b.id === this.activeThiefEvent?.ballId,
        );
        if (!ball || !ball.container) {
            console.warn(
                "[FloatingBallMathGameScene] Target ball not found or has no container",
            );
    ...
    evaluateBlockDecision(): boolean {
        ...
        const isBadBall =
            newScore < 0 || newScore > this.currentEquation.target * 2;
    ...
    evaluateAdaptDecision(): boolean {
        ...
        console.log("[FloatingBallMathGameScene] Adapt Decision Evaluation:", {
            ...
            result:
                !isBadBall &&
                this.activeThiefEvent!.targetLane === this.currentLane,
        });

        // Adapt is correct if ball is good and new lane is accessible
        return (
            !isBadBall && this.activeThiefEvent!.targetLane === this.currentLane
        );
    }
    ...
    hideBlockButton() {
        ...
        this.blockTimerEvent = undefined as any;
    ...
    update(time: number, delta: number) {
        ...
        if (!this.isPaused && this.physicsEnabled) {
            ...
            const activeBalls = this.balls.filter(
                (ball) => ball && ball.container && !ball.isCollected,
            );

            // Regenerate balls if all collected (even after timeout)
            if (activeBalls.length === 0 && !this.isLocked) {
                console.log(
                    "[FloatingBallMathGameScene] No active balls, regenerating solvable ball set",
                );
        ...
        if (this.activeThiefEvent && this.armSprite.visible && !this.isPaused) {
            const trackedBall = this.balls.find(
                (b) => b.id === this.activeThiefEvent!.ballId,
            );
            if (
                trackedBall &&
                !trackedBall.isCollected &&
                trackedBall.container
            ) {
                ...
                if (trackedBall.y > fiftyPercentHeight) {
                    console.log(
                        `[FloatingBallMathGameScene] Ball at Y=${trackedBall.y} is below 50% (${fiftyPercentHeight}) - executing adapt`,
                    );
    ...
    async collectBall(ball: FloatingBall) {
        console.log(
            `[FloatingBallMathGameScene] Ball collected - id: ${ball.id}, operator: ${ball.operator}, value: ${ball.value}`,
        );
        ...
        if (this.activeThiefEvent && this.activeThiefEvent.ballId === ball.id) {
            console.log(
                "[FloatingBallMathGameScene] Target ball collected during thief event - hiding arm",
            );
            this.armSprite.setVisible(false);
        ...
        if (ball.isSolvable) {
            console.log(
                "[FloatingBallMathGameScene] Solvable ball collected, spawning replacement",
            );
            this.spawnReplacementSolvableBall();
        }
        ...
        if (this.currentScore < 0) {
            console.log(
                "[FloatingBallMathGameScene] Score dropped below 0, ending game with loss",
            );
        ...
        if (this.currentScore === this.currentEquation.target) {
            ...
            this.currentScore < 0 ||
            this.currentScore > this.currentEquation.target * 2
        ) {
    ...
    applyOperation(ball: FloatingBall) {
        ...
        console.log(
            `[FloatingBallMathGameScene] Applied operation: ${oldValue} ${ball.operator} ${ball.value} = ${this.currentScore}`,
        );
        this.updateCurrentDisplay();
    }
    ...
    handleSuccess() {
        ...
        const childrenWithSign = this.floatboatController.getChildrenWithSign();
        const targets = childrenWithSign
            ? [childrenWithSign, this.targetDisplay, this.currentDisplay]
            : [this.targetDisplay, this.currentDisplay];
        this.tweens.add({
            targets: targets,
            scale: 1.2,
            duration: 200,
        ...
        this.time.delayedCall(1500, () => {
            ...
            if (
                this.correctEquations >= this.currentLevelConfig.totalEquations
            ) {
                this.endLevel();
            } else {
                this.generateNewEquation();
        ...
        this.timerEvent = this.time.addEvent({
            ...
            callback: () => {
                ...
                if (remainingMs <= 0 && !this.continuedAfterTimeout) {
                    console.log(
                        "[FloatingBallMathGameScene] Timer reached 0, ending game with loss",
                    );
                    this.endGameWithLoss();
                }
            },
